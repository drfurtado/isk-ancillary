...
Follow the instructions in [this video](https://youtu.be/dE-fQzM86Ew) tutorial to learn how to run a z-test[^3] in jamovi.

[^3]: This can be accomplished by installing a module.
...
-   The data are continuous (not discrete);
-   The data follow the normal probability distribution;
-   Sample is a simple random sample from the population[^4].

[^4]: Each individual in the population has an equal probability of being selected in the sample.
...
-   compare the observed p-value[^5] with the alpha value ($\alpha$).

    -   if the calculate p-value is less than the $\alpha$, reject the $H_0$

[^5]: Value calculated by the statistical package; i.e., jamovi, SPSS
...
Follow the instructions in [this video](https://youtu.be/dE-fQzM86Ew) tutorial to learn how to run a z-test[^6] in jamovi.

[^6]: This can be accomplished by installing a module.
